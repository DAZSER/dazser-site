<p>Jani-King has the experience and know-how to deliver a clean facility.</p>
<p><img src="images/stock/r26.jpg" class="right" />To provide the highest in quality services, Jani-King has developed quality assurance communication tools to make certain every facility cleaned by Jani-King meets our standard of excellence.
</p>
<h2>Our communication tools include:</h2>
<ul>
	<li>Past Performance Reports</li>
	<li>Fax-a-Comment</li>
	<li>Communication Log</li>
	<li>Facility Inspections</li>
	<li>Customer Evaluation Forms</li>
	<li>Customer Request Memo Pad</li>
</ul>
<p>Jani-King regional offices review past work and conduct comprehensive facility inspections to make sure everything is done to meet customers' specifications.</p>

<h2>Satisfied clients include:</h2>
<ul>
<li>Salt Lake City 2002 Winter Olympic Games</li>
	<li>America Online</li>
	<li>Continental Airlines</li>
	<li>IBM</li>
	<li>Motorola</li>
	<li>Pepsico</li>
	<li>Westin Hotel &amp; Resorts</li>
</ul>
<h2 style="text-align:left;"><p><img src="images/stock/17.jpg" alt="" class="left" style="margin:2em 1.5em;"/></p>Our cleaning expertise spans a variety of businesses, large and small, including:</h2>
<ul>
    <li>Retail Locations</li>
    <li>Office Buildings</li>
    <li>Country Clubs</li>
    <li>Hotels/Resorts</li>
    <li>Restaurants</li>
    <li>Casinos</li>
    <li>Timeshares</li>
    <li>Medical Facilities</li>
    <li>Schools & Universities</li>
    <li>Entertainment & Sporting Venues</li>
    <li>Industrial Facilities</li>
</ul>

<p>Trained in the latest cleaning techniques and armed with cutting-edge cleaning equipment, we provide customized service. And because our clients have their own individual cleaning needs, we tailor our cleaning programs to fit their specifications.</p>

<p>We know you have many choices when it comes to choosing a cleaning company. You need flexible solutions to meet today's evolving facility demands. You have to choose the company that can best provide the quality services you need. Jani-King is the right choice. We recognize each company's unique culture. We will furnish progressive, cutting-edge solutions, and continually communicate with you to ensure on-going success.</p>